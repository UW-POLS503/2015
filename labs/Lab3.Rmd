...
mean <- summarise_each(iver, funs(mean), - cty)
sd <- summarise_each(iver, funs(sd), - cty)
min <- summarise_each(iver, funs(min), - cty)
max <- summarise_each(iver, funs(max), - cty)
median <- summarise_each(iver, funs(median), - cty)
...
iver_summary2 <- iver %>%
	summarise_each(funs(mean, sd, min, max, median), - cty) %>%
...
```{r}
lm_multi <- lm_multi <- lm(povred ~ lnenp + maj + pr, data = iver, x = TRUE)
lm_multi$x
```
...
```{r}
lm_multi_interact <- lm(povred ~ lnenp*maj + pr, data = iver, x = TRUE)
lm_multi_interact$x
```
...
```{r}
lm_multi_log <- lm(povred ~ log(enp) + maj + pr, data = iver)
...
```{r}
lm_multi_sq <- lm(povred ~ enp + I(enp^2) + maj + pr, data = iver)
summary(lm_multi_sq)
```
...
mydata <- data.frame(somenumbers = seq(1,50,7),
										 ...
										 is.silly = c(TRUE, FALSE, FALSE, TRUE, FALSE, FALSE,
										 						  TRUE, TRUE))
mydata
```
...
```{r}
predict(lm_multi, newdata = data.frame(lnenp = log(2), maj = 1, pr = 0),
				interval = "confidence")
predict(lm_multi, newdata = data.frame(lnenp = log(seq(1:5)), maj = 1, pr = 0),
				interval = "confidence")
```
or
```{r}
xnew <-  data.frame(lnenp = log(seq(1:5)), maj = 1, pr = 0)
predict(lm_multi, newdata = xnew, interval="confidence")
```
or
```{r}
data.frame(lnenp = log(seq(1:5)), maj = 1, pr = 0) %>%
...
```{r}
yhyp <- data.frame(lnenp = iver$lnenp, 
									 maj = mean(iver$maj), 
									 pr = mean(iver$pr)) %>%
	predict(lm_multi, newdata = . , interval = "confidence")
```
...
```{r}
plot <- plot +
	geom_ribbon(aes(ymin = yhyp[ , 2], ymax = yhyp[ , 3]),
							alpha = (1 / 3)) + geom_line(aes(x = lnenp, y = yhyp[ , 1]))
plot
```
