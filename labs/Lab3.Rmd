...
```{r}
lm_multi <- lm_multi <- lm(povred ~ lnenp + elec_sys, data = iver, x = TRUE)
lm_multi$x
```
...
```{r}
predict(lm_multi, newdata = data_frame(lnenp = log(2), elec_sys = "maj"),
				interval = "confidence")
predict(lm_multi, newdata = data_frame(lnenp = log(seq(1:5)), elec_sys = "maj"),
				interval = "confidence")
```
or
```{r}
...
```{r}
yhyp <- data_frame(lnenp = iver$lnenp, 
									 elec_sys = "maj") %>%
	predict(lm_multi, newdata = . , interval = "confidence")
```
...
```{r}
plot + geom_text(size = 3) +	theme_minimal()
```
