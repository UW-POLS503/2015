...
The `logBound()` and `logitBound()` functions can be loaded by running the following code
```{r eval=FALSE}
source("https://pols503.github.io/pols_503_sp15/hw/hw4-functions.html")
```
After loading the library, the above regression is as simple as:
```{r eval=FALSE}
