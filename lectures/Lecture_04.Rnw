...
\newcommand{\thetitle}{Sum of Squares, Total Sum of Squares, and $R^{2}$}
\date{April 20, 2015}
\title{\thetitle{}}
\hypersetup{
  pdftitle={\thetitle{}},
...
\begin{frame}
  ...
  \item But sample is random, how do we characterize the uncertainty in our estimates?
  \end{itemize}
...
\begin{frame}
  ...
  \begin{equation*}
    \bar{x}_{D} - \bar{x}_{R} \sim N\left(\frac{\mu_{D} - \mu_{R}}{\sqrt{\frac{\sigma_{R}^{2}}{n_{R}} + \frac{\sigma_{D}^{2}}{n_{D}}}} \right)
  \end{equation*}
...
\begin{frame}
  ...
  \item $p$-value is the probability of data (test statitics) equal or more extreme than the sample
...
\begin{frame}
  ...
  \item Which is distributed
    \begin{equation*}
      ...
      \right)
    \end{equation*}
  \item But don't know $\sigma_{R}^{2}$ and $\sigma_{D}^{2}$.
  \item Use $s_{R}^{2}$, $s_{D}^{2}$, $t$ distributed Student's t to acccount for uncertainty from estimating standard deviations.
  \end{itemize}
...
\begin{frame}
  ...
  \item Tests generally focus on Type I error
    \begin{itemize}
    \item ``conservative'' -- is it really?
...
\begin{frame}
  ...
  \item Problems with \textbf{both}
    ...
    \item ``File drawer problem'', ``fishing'': even if it makes sense on an individual test, multiple testing within a research project + selecting on significant results can result in
    \end{itemize}
  \item Problems with signficance tests that CI overcome
    \begin{itemize}
    \item tests are ``weak'' -
    \item confidence intervals focus more on substantive significance (parameter values); $p$-values ignore all substantive signifance.
    \end{itemize}
  \end{itemize}
...
\begin{frame}
  ...
  \only<2>{\textbf{My Claim:} Even if using frequentist tests $p(D | H)$, a paper
    assigns prior probilities to hypotheses, e.g. lit review, logical arguments, etc. to make a Bayesian argument, $p(D | H) p(H)$.}
...
\begin{frame}
  ...
  \item Confidence interval is
    ...
    \end{equation*}
  \item standard error is $V(B)$ with $\hat{\sigma}_{\epsilon}^{2}$ as an estimate of $\sigma_{\epsilon}$.
  \end{itemize}
\end{frame}    
...
\begin{frame}
  ...
  Estimate $\sigma_{\epsilon}^{2}$ from the regression errors
  \begin{equation*}
    \hat{\epsilon} = \frac{\sum E_{i}^{2}}{n - k - 1}
  \end{equation*}
  \begin{itemize}
  \item  $n - k - 1 = \text{(observations)} - \text{(variables)} - \text{(intercept)}$ is the degrees of freedom.
...
\begin{frame}
  ...
  \begin{itemize}
  \item analagous to the bivariate version
    \begin{itemize}
    \item calculates all standard errors simultaneously
    \item covariances: $\vc{b}_{i}$ and $\vc{b}_{j}$ can be correlated
    \end{itemize}
  \end{itemize}
...
\begin{frame}
  ...
  \begin{equation*}
    SE(B_{j}) =  \sqrt{se^{2} (X' X)^{-1}_{jj}} = \frac{1}{\sqrt{1 - R^{2}_{j}}} \frac{se}{\sqrt{\sum {(x_{ij} - \bar{x}_{j})}^{2}}}
  \end{equation*}
...
\begin{frame}
  ...
  \item Common example:
    ...
    \item Correct procedures:
      \begin{itemize}
      \item Sigificance test with $H_{0}: \beta_{A} = \beta_{B}$
...
\begin{frame}
  ...
  \item $p$-values only show statistical signifiance, not substantive significance.
...
\begin{frame}
  ...
  \begin{itemize}
  \item Some slides derived from Christopher Adolph \textit{Inference and Interpretation of Linear Regression}. Used with permission. <http://faculty.washington.edu/cadolph/503/topic4.pw.pdf>
