...
\begin{tikzpicture}[>=latex,scale=1.2]
  ...
  \node (R) at (0, 0) {$(-\infty, \infty)$} ;
  \node (p) at (5, 0) {$(0, 1)$};

  \draw[->] (p) to [bend right=45]
  node[midway,below] {$\logit(x) = \log \left( \frac{x}{1 - x} \right)$}
  (R);
  \draw[->] (R) to [bend right=45]
  node[midway,above] {$\logit^{-1}(x) = \frac{e^{x}}{e^{x} + 1} = \frac{1}{1 + e^{-x}}$}
  (p);
...
\begin{frame}
  ...
  \begin{itemize}
  \item Models $\Pr(Y_{i} = 1)$ instead of $Y_i$ directly
  \item Although $X_{i} \beta$ is linear, $\logit(x)$ is non-linear.
  \end{itemize}

\end{frame}

\begin{frame}
  \frametitle{Logit Model as a Linear Function of Log Odds}
...
\begin{frame}
  ...
  \begin{equation*}
    \hat{\beta} &= \argmin_{b} \sum_{i} (y_{i} \log P_{i} +  (1 - y_{i}) \log (1 - P_{i}) \\
