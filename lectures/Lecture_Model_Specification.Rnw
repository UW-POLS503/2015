...
\begin{frame}
  ...
  \begin{equation*}
    \hat{\sigma} = S_{E} = \sqrt{\frac{\sum E_{i}^{2}}{n - k - 1}}
  \end{equation*}
  
  \begin{itemize}
    
  \item $S_E$ is at least as useful to report as $R^{2}$
  \item $S_{E}$ is the average error $E_{i}$
  \item On the same scale as $y$. Substantive significance can be clearer.
  \item Smaller $S_{E}$ is better
  \end{itemize}
...
\begin{frame}
  ...
  \item For a normal linear model, the likelihood is
    \begin{equation*}
      p(y) = \prod_{i} N(y_{i}| X_{i} \beta, \sigma_{\epsilon}^{2}) = \prod_{i} \frac{1}{\sigma_{\epsilon}} \exp
      \left(
        - \frac{(y_{i} - x'_{i} \beta)^{2}}{2 \sigma^{2}_{\epsilon}}
      \right)
      =  \prod_{i}
      \frac{1}{\sigma_{\epsilon} \sqrt{2 \pi}}
      \left(
        - \frac{\epsilon_{i}^{2}}{2 \sigma^{2}_{\epsilon}}
      \right)
    \end{equation*}
  \item For computational stability (the product of probabilities is a small number), the log likelihood is usually used
    \begin{equation*}
      \log p(y) \propto \sum_{i} \epsilon_{i}^{2}
    \end{equation*}
  \item The 
  \end{itemize}
...
\begin{frame}
  ...
  \begin{itemize}
  \item Information criteria are the Log Likelihod + a penalty for complexity
  \item The two Most common are AIC and BIC:
    \begin{align*}
      AIC_j &= -2 \log L(\hat{\theta}) + 2 k \\
...
\begin{frame}
  ...
  \item Usual methods:
    ...
    \item 5-- or 10--fold cross-validation
    \end{itemize}
  \item Best model minimizes MSE
  \end{itemize}
