...
\begin{frame}
  ...
  \begin{block}{Diagnosing}
    \begin{itemize}
    \item Plot $E$ against $\hat{Y}$ - sometimes $E^2$ or $|E|$ or $\log \hat{Y}$.
    \item R function \verb|residualPlots|
    \end{itemize}
  \end{block}

  \begin{block}{Tests}
    \begin{itemize}
    \item All tests of the form regress residuals on functions of $X$ (Breusch-Pagan, White) \verb|car::ncvTest|
