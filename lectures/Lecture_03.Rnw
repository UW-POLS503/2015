...
\begin{frame}
  ...
  \begin{itemize}
  \item State $B$ in terms of $\cov$ and $\var$? 
  \item State $B$ in terms of $\cor(X, Y)$ and $\sd$?
...
\begin{frame}
\frametitle{Least Squares coefficients are unidentified if $X' X = 0$}  
...
\begin{frame}
  ...
  The sampling distributions of $B_{k}$ given $Y_{i} = \alpha + \beta X_{i} + \epsilon_{i}$
  \begin{itemize}
