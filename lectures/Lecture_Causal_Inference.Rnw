...
\begin{frame}
  ...
  Consider two individuals, Annie and Jeff.

  The effects of hospital on
  \begin{itemize}
  \item (health of Annie if he goes hospital) - (health if she does not)
...
\begin{frame}
  \begin{tabular}{lcccc}
    & No Hospital & Hospital & Treatment & Outcome \\
...
\begin{frame}
  ...
  We cannot simply compare group averages to get a causal effect
...
\begin{frame}
  ...
  \begin{align*}
    ...
    &\quad = \E(Y_{i}(1) |D_{i} = 1) - \E(Y_{i}(1) | D_{i} = 1) && \text{$\tau_{ATT}$, Avg. Treatment on the treated} \\
