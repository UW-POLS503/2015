...
```
Now we generate a new dataset with those dummy variables and with the missing values replaced by 0s.
```{r}
UN_with_dummies <- 
  UN %>%
...
Let's see how one of these imputed datasets looks:
```{r}
...
The plots we just made suggest **Amelia** is working weel, but let's look at a couple of diagnostics that **Amelia** suggests:
...
```
The second method is to compare the density of the imputed values to the density of the observed values for each variable.
If they do not appear too different, then the imputation procedure has worked well.
It is not necessary that the marginal distribution of the imputed variable should match that of the observed values, but if they appear significantly different, then you should think about whether this could be expected or whether your imputation model is missing something.
```{r}
for (var in c("GDPperCapita", "contraception", "illiteracyFemale")) {
...
The function `mi.meld` combines the point estimates and standard errors from the analysis on each imputation:
```{r}
