...
The function `sdcor2cov` calculates the covariance matrix from a standard deviation
...
For all of these, focus on the coefficients of the

- The bias of each $\hat{\beta}_j$?. How different is `estimate_mean` ($\hat\beta$)
...
3. Drop the Intercept term
4. Sort by term and sample size to make it easier to understand
...
\end{aligned}
$$
The covariates, $x_j$, are independent, with means, $\mu_{x,j} = 0$, and standard deviations, $s_{x,j} = 1$ for all variables.
...
To run a simulation for a single value of $\rho$:
```{r, results='hide'}
n <- 1024
mu_X <- c(0, 0, 0)
s_X <- c(1, 1, 1)
...
In these simulations, we will use a population model of
$$
\begin{aligned}[t]
...
To run `sim_lin_norm_heterosked` for a single value of $\gamma_1$:
```{r}
n <- 1024
mu_X <- c(0, 0)
s_X <- c(1, 1)
...
```{r}
gamma_1 <- 0
gamma <- c(1.5, gamma_1, 0)
```
Given these values, run `sim_lin_norm_heterosked`:
```{r results='hide'}
sim_lin_norm_heterosked(iter = iter, n = n, mu_X = mu_X,
                        s_X = s_X, R_X = R_X, beta = beta, gamma = gamma)
```

Run `sim_lin_norm_heterosked` for each of these values of $\gamma_1$, which will vary the level of heteroskedasticity:
...
How does the level of heteroskedasticity affect:
