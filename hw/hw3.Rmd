...
$$
R_X = \begin{matrix} 1 & 0 & 0 \\ 0 & 1 & 0 \\ 0 & 0 & 1 \end{matrix}
$$
...
beta <- c(0, 1, 1, 1)
```
And we'll set the standard deviation of the regression errors so that that the $R^2$ of the regression is 0.5, meaning that the regression explains half the variance in $y$.
```{r}
sigma <- 1.7
```
