...
```{r}
sim_lin_norm_heterosk <- function(m, n, mu_X, s_X, R_X, beta, sigma, gamma) {
  assert_that(length(s_X) == length(mu_X),
              ncol(R_X) == nrow(R_X),
              ncol(R_X) == length(mu_X),
              length(beta) == (length(mu_X) + 1),
...
```{r}
knit_formatR <- function(source, ...) {
  tmpfn <- tempfile()
  purl(source, output = tmpfn, documentation = 2L)
  tidy_source(source, ...)
}
```
...
Run `sim_lin_norm_heterosk` and vary the amount of heteroskedasticity by varying $\gamma_1$; let $\gamma_1 = 0$ (no heteroskedasticity), and $\gamma_1 = 1$ and answer how heteroskedasticity affects the following:
...
Before running simulations, try a few
