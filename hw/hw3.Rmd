...
```{r}
sim_lin_norm <- function(sims, n, mu_X, s_X, R_X, beta, sigma) {
  ...
  # Generate an X
  X <- mvrnorm(n, mu_X, sdcor2cov(s_X, R_X), empirical = TRUE)
...
For all these simulations use 3 covariates that have $\mu_X = (0, 0, 0)$, $s_X = (1, 1, 1)$, and are are independent.
Let $\beta = c(0, 1, 1, 1)$, and $\sigma = 1.7$

Run several simulations varying the sample size to determine the following:
...
Now, we will evaluate the propoerties of OLS estimates when covariates are correlated.
Use `sim_lin_normal` and the previous settings, but adjust the correlation matrix between the variables. Consider the following correlations. 
...
In these simulations, there are 4 variables. 
All variables should have mean 0, standard deviation 1, and be independent.
Let $\beta = (0, 0, 0.1, 0.5, 1)$, and let $\sigma = 1.9$, which is an $R^2$ of about 0.25.
Run 1,024 iterations of `sim_lin_norm` and use the results to answer the following questions:
...
```{r}
sim_lin_norm_omitted <- function(sims, n, mu_X, s_X, R_X, beta, sigma,
                                 omit = integer(0)) {
  assert_that(length(s_X) == length(mu_X),
              ncol(R_X) == nrow(R_X),
              ncol(R_X) == length(mu_X),
...
```{r}
sim_lin_norm_heterosk <- function(sims, n, mu_X, s_X, R_X, beta, sigma, gamma) {
  assert_that(length(s_X) == length(mu_X),
              ncol(R_X) == nrow(R_X),
              ncol(R_X) == length(mu_X),
              length(beta) == (length(mu_X) + 1),
...
```{r}
sim_lin_norm_truncated <- function(sims, n, mu_X, s_X, R_X, beta, sigma, truncation = 0.5) {
  X <- MASS::mvrnorm(n, mu_X, sdcor2cov(s_X, R_X), empirical = TRUE)
  simulations <- list()
  p <- progress_estimated(sims, min_time = 2)
  for (j in 1:sims) {
...
```{r}
sim_lin_norm_ma1 <- function(sims, n, mu_X, s_X, R_X, beta, sigma,
                             rho = 0.5, rho_X = 0) {
  assert_that(length(s_X) == length(mu_X),
              ncol(R_X) == nrow(R_X),
              ncol(R_X) == length(mu_X),
