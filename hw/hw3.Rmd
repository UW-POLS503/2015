...
A few of the functions will use **assertthat**, which contains functions to test for errors in order to catch bugs.
```{r}
library("MASS")
library("ggplot2")
library("dplyr")
...
The outline of a simulation is:
...
First, we will write code to generate a single sample and run OLS on it.
Then, we will wrap that that code in a for loop to be able to run a simulation with many samples, and wrap that in a function in order to be able to easily change parameters of the simulation without repeating ourselves.
...
However, drawing one sample is not enough for a simulation study.
We want to repeat this many times in order to generate a sampling distribution in order to evaluate how well the estimator works.
...
```
Let's draw 1,024 samples using the values that we used before:
```{r}
sim0 <- sim_lin_norm(1024, beta, mvrnorm(16, mu_X, Sigma_X), sigma)
```
...
Now with all the simulations in hand, we can summarize the simulation results and compare them to the population parameters.
The following funciton takes a the data frame generated by the simulation function and a vector of the original parameters, and generates a summary data frame with one row per coefficient.
```{r sim_summary}
...
```
Using the previous results, the a summary is 
```{r}
summarize_sims(sim0, beta)
```
...
In this problem, we are going to generate samples from a population 

Generate an $X$ with $k = 3$ *independent* variables with $\mu_X = (0, 0, 0)$, $\s_X = (1, 1, 1)$, for sample sizes of 8, 64, 512, 4096.
```{r}
# mu_X <- c(0, 0, 0)
# s_X <- c(1, 1, 1)
# R_X <- diag(3) # 3 x 3 identity matrix
# Sigma_X <- sdcor2cov(s_X, R_X)
# X <- mvrnom(???, mean = c(0, 0, 0), empirical = TRUE)
```
...
How does the variance change? Consider $\hat{\beta}_1$, $\hat{\beta}_2$, and $\hat{\beta}_3$.
...
```{r}
group_by(sim1, term) %>%
  summarize(pct = mean(p.value < 0.05))
```
...
```{r}
ggplot(group_by(sim1, term) %>%
      filter(p.value < 0.05), aes(x = estimate)) +
  geom_density() +
  geom_rug() + 
  facet_wrap( ~ term, scale = "free")
...
Repeat those analyses using a smaller sample size, $n = 32$, and a larger sample
size $n = 1024$.
...
The following function, `sim_lin_norm_omitted` allows you omit variables when estimating the sample regressions. 
The arguments to this function are the same as `sim_lin_norm` except for the the argument `omit`, which takes a vector of the columns to omit from $X$ when estimating the sample regression.
```{r}
...
The following function is similar to `sim_lin_norm`, but all `y_i` where all `y` less than a given quantile are dropped before the regression is run. 
This is specified by the `truncation` argument.
The default value `truncation = 0.5` means all values of $y$ less than the median are dropped before running a regression.
```{r}
...
Although only one variable is used, you can still use `mvrnorm` to sample $X$.
Let $\beta = (0, 1)$ and $\sigma = 1$.

How does varying the sample size affect the bias and variance of $\hat{\beta}$? 
Try values of $n$ of 8, 64, 128, 1024.
