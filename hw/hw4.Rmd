...
a. Estimate the model
    $$
    \begin{aligned}[t]
    \log(\mathtt{finish}_i) &= \beta_0 + \beta_1 \mathtt{year}_i + \beta_2 \mathtt{women}_i + \beta_3 \mathtt{women}_i \mathtt{year}_i  + \epsilon_i
    \end{aligned}
    $$
...
b. Create the same plot as in the previous part for the model
    $$
    \begin{aligned}[t]
    \log(\mathtt{finish}_i) &= \beta_0 + \beta_1 \mathtt{year}_i + \beta_2 \mathtt{women}_i + \beta_3 \mathtt{year}_i × \mathtt{women}_i + \epsilon_i
    \end{aligned}
    $$
...
model:
    $$
    \begin{aligned}[t]
    \log(\mathtt{Finish}_i) &= \beta_0 + \beta_1 \mathtt{year}_i + \beta_2 \mathtt{women}_i + \beta_3 \mathtt{year}_i × \mathtt{women}_i \\
...
```{r}
rossoil <- read.csv("http://pols503.github.io/pols_503_sp15/data/rossoildata.csv") %>%
   arrange(id1, year) %>%
   group_by(id1) %>%
